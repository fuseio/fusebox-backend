...
import { TransferDto } from './dto/trasfer.dto';
...
export class TransferService {
    ...
    async transferPost(transferDto: TransferDto) {
        const contract = new this.web3Provider.eth.Contract(erc20 as any, transferDto.fromAddress)
        console.log(this.transferEventHash)
        let mappedlog
        const logs = await this.queryCreate(transferDto)
        const promisesLog = logs.map(async (log) => await this.processEvent(log as any))
        mappedlog = await Promise.all(promisesLog)
        return mappedlog
    }
    ...
    async queryCreate(transferDto: TransferDto) {
        ...
        if (transferDto.minted && toAddress) {
